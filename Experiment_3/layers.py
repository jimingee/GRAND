...
class ConstantODEblock(ODEblock):
    def __init__(self, odefunc, regularization_fns, opt, w_config, data, device, t=torch.tensor([0, 1])):
        ...
        self.set_tol(w_config)

    def forward(self, x, w_config):
        t = self.t.type_as(x)
        ...
        ## odeint -> differential 수행
            state_dt = integrator(
                ...
                options=dict(step_size=w_config.step_size, max_iters=self.opt['max_iters']),
                adjoint_method=self.opt['adjoint_method'],
                adjoint_options=dict(step_size = w_config.adjoint_step_size, max_iters=self.opt['max_iters']),
                atol=w_config.tol_scale,
                rtol=w_config.tol_scale,
                adjoint_atol=w_config.tol_scale_adjoint,
                adjoint_rtol=w_config.tol_scale_adjoint)
        else:
            state_dt = integrator(
                func, state, t,
                method=self.opt['method'],
