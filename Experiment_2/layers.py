...
class ConstantODEblock(ODEblock):
    def __init__(self, odefunc, regularization_fns, opt, w_config, data, device, t=torch.tensor([0, 1])):
        ...
        self.odefunc = odefunc(self.aug_dim * w_config.hidden_dim, self.aug_dim * w_config.hidden_dim, opt, w_config, data, device)
        if opt['data_norm'] == 'rw': 
...
class LaplacianODEFunc(ODEFunc):
    ...
    def __init__(self, in_features, out_features, opt, w_config, data, device):
        ...
        self.w = nn.Parameter(torch.eye(w_config.hidden_dim)) 
        self.d = nn.Parameter(torch.zeros(w_config.hidden_dim) + 1)
