...
def train(model, w_config, optimizer, data, pos_encoding=None):
    ...
    model.resetNFE()
    # model.getNFE() : self.odeblock.odefunc.nfe + self.odeblock.reg_odefunc.odefunc.nfe
    # model.resetNFE() : self.odeblock.odefunc.nfe=0, self.odeblock.reg_odefunc.odefunc.nfe=0
...
def main(opt, w_config):
    ...
    optimizer = build_optimizer(parameters, w_config.optimizer, learning_rate=w_config.learning_rate, weight_decay=opt['decay'])
...
sweep_config = {
    ...
    'parameters':{
        ...
        'epoch':{'min':1, 'max':100},
        'hidden_dim': {'min':20, 'max':100}, 
        'optimizer': {'values' : ['adamx', 'adam','sgd']}
    }
...
def set_config():
    ...
    #parser.add_argument('--dropout', type=float, default=0.046878964627763316, help='Dropout rate.')
    #parser.add_argument('--learning_rate', type=float, default=0.022924849756740397, help='Learning rate.')
    ...
    parser.add_argument('--method', default = 'rk4',type=str, help="set the numerical solver: dopri5, euler, rk4, midpoint") 
...
def run_sweep(config = None):
    ...
    wandb.run.name = f'GNN_args_{cnt}'
    #wandb.run.name = f'GNN_args-epoch_{opt.epoch}-lr_{opt.learning_rate}'
    w_config = wandb.config
    cnt += 1

    main(opt, w_config)
    
sweep_id = wandb.sweep(sweep=sweep_config, project = 'Sweep-GNN-params', entity='jimin-choi')
